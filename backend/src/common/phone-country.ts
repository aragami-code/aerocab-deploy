...
export const PHONE_PREFIX_MAP: Record<string, string> = {
...
const SORTED_PREFIXES = Object.keys(PHONE_PREFIX_MAP)
 ...
 */
export function extractCountryFromPhone(phone: string): string | null {
  if (!phone?.startsWith('+')) return null;
  for (const prefix of SORTED_PREFIXES) {
    if (phone.startsWith(prefix)) return PHONE_PREFIX_MAP[prefix];
  }
  return null;
}
