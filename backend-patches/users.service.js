...
let UsersService = class UsersService {
    constructor(prisma, settings, notifications, trustScoreService) {
        this.prisma = prisma;
    ...
    async updateProfile(userId, dto) {
        const user = await this.prisma.user.update({
            where: { id: userId },
            data: Object.assign(Object.assign(Object.assign(Object.assign({}, (dto.name !== undefined && { name: dto.name })), (dto.email !== undefined && { email: dto.email })), (dto.phone !== undefined && { phone: dto.phone })), (dto.language !== undefined && { language: dto.language })),
            select: {
                id: true,
                phone: true,
                name: true,
    ...
    async deleteAccount(userId) {
        ...
        if (user.status === 'deleted')
            throw new common_1.BadRequestException('Compte déjà supprimé');
    ...
    async getLoyaltyStatus(userId) {
        ...
        const earnedTotal = (_a = totalEarned._sum.points) !== null && _a !== void 0 ? _a : 0;
    ...
    async updateLoyaltyTier(userId) {
        ...
        const pts = (_a = totalEarned._sum.points) !== null && _a !== void 0 ? _a : 0;
        const newTier = pts >= thresholds.platinum ? 'platinum' :
        ...
        if (user && user.loyaltyTier !== newTier) {
            ...
            const tierLabels = { bronze: 'Bronze', silver: 'Argent', gold: 'Or', platinum: 'Platine' };
            this.notifications.sendToUser(userId, `${meta.emoji} Niveau ${tierLabels[newTier]} atteint !`, `Félicitations ! Vous avez atteint le niveau ${tierLabels[newTier]}. Découvrez vos nouveaux avantages.`).catch(() => { });
        }
    }
    /** Taux de cashback bonus selon le niveau (en %) */
    getTierCashbackRate(tier) {
        var _a;
        const rates = { bronze: 0, silver: 5, gold: 10, platinum: 15 };
        return (_a = rates[tier]) !== null && _a !== void 0 ? _a : 0;
    }
    /** Récupère le tier d'un passager — utilisé par BookingsService */
    async getPassengerTier(userId) {
        var _a;
        const user = await this.prisma.user.findUnique({ where: { id: userId }, select: { loyaltyTier: true } });
        return ((_a = user === null || user === void 0 ? void 0 : user.loyaltyTier) !== null && _a !== void 0 ? _a : 'bronze');
    }
    ...
    async getPassStatus(userId) {
        const [enabledRaw, priceRaw, durationRaw, trialRaw, graceDaysRaw] = await Promise.all([
            ...
            this.settings.get('access_pass_trial_days', '0'),
            this.settings.get('access_pass_grace_days', '0'),
        ]);
        const required = enabledRaw === 'true';
        const price = parseInt(priceRaw, 10) || 5000;
        const durationDays = parseInt(durationRaw, 10) || 30;
        const trialDays = parseInt(trialRaw, 10) || 0;
        const graceDays = parseInt(graceDaysRaw, 10) || 0;
        if (!required) {
            return { required: false, active: true, inGrace: false, daysLeft: 0, price, durationDays, trialDays };
        }
        const user = await this.prisma.user.findUnique({
            where: { id: userId },
            select: { passExpiresAt: true, passType: true, createdAt: true },
        });
        if (!user) throw new common_1.NotFoundException('Utilisateur introuvable');
        const now = new Date();
        const expiry = user.passExpiresAt;
        ...
        const trialEnd = trialDays > 0 ? new Date(user.createdAt.getTime() + trialDays * 86400000) : null;
        const inTrial = !!trialEnd && trialEnd > now;
        let daysLeft = 0;
        if (hasValidPass) {
            daysLeft = Math.ceil((expiry.getTime() - now.getTime()) / 86400000);
        } else if (inGrace && graceEnd) {
            daysLeft = Math.ceil((graceEnd.getTime() - now.getTime()) / 86400000);
        }
        return {
            required,
            active: hasValidPass || inTrial,
            inGrace,
            daysLeft,
            price,
            durationDays,
